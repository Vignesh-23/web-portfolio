        ...
        for (let i = 0; i < this.stars.length; i++) {
            for (let j = i + 1; j < this.stars.length; j++) {
                ...
                if (distance < this.maxDistance) {
                    const opacity = (this.maxDistance - distance) / this.maxDistance * 0.5;
    ...
    createMouseStars() {
        ...
        if (Math.random() < 0.1) {
            ...
            setTimeout(() => {
                ...
                star.style.transform = 'scale(0.5) translate(' + 
...
function setupInteractiveTimeline() {
    ...
    timelineBars.forEach(bar => {
        ...
        bar.addEventListener('mouseenter', function() {
            ...
            if (tech) {
                ...
                detailTech.innerHTML = techArray.map(t => 
    ...
    document.addEventListener('click', function(e) {
        if (!e.target.closest('.timeline-bar') && !e.target.closest('.timeline-details-panel')) {
            if (activeBar) {
                activeBar.classList.remove('active');
                activeBar = null;
            }
...
function setupContactForm() {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            ...
            if (formMessage) {
                formMessage.innerHTML = 'Thank you for your message! Your email client should open with the pre-filled message.';
